...
let getGamesFromVanityURLs = ( communityURLs ) => {
  ...
  .then( usersPlayTimes => {
    ...
    return [ util.mergeGamePlaytimes( usersPlayTimes ), communityURLs ];
  })
  .catch( err => {
    throw err;
  });
...
function getMostPopularGameByOwnership( vanityUrls ) {
  ...
  .then( mergedGames => {
    result.vanityUrls = mergedGames[ 1 ];

    return util.findMostPopularGameByOwnership( mergedGames[ 0 ] );
  })
  .then( popularGame => {
    result.result = popularGame;
...
function getMostPopularGameByPlaytime( vanityUrls ) {
  ...
  .then( mergedGames => {
    ...
    return util.findMostPopularGameByPlaytime( mergedGames[ 0 ] );
  })
  .then( popularGame => {
    result.result = popularGame;
