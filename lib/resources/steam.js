...
const vanityRegEx = /(?:http(?:s|):\/\/|)(?:www.|)steamcommunity\.com\/id\/([a-zA-Z0-9]*)(?:\/|)/g;
...
function getMostPopularGameByOwnership( vanityUrls ) {
  ...
  .then( mergedGames => {
    ...
    scopeBreakingResultBullshit.result = JSON.stringify( util.findMostPopularGameByOwnership( mergedGames ) );
...
function getMostPopularGameByPlaytime( vanityUrls ) {
  ...
  .then( mergedGames => {
    ...
    scopeBreakingResultBullshit.result = JSON.stringify( util.findMostPopularGameByPlaytime( mergedGames ) );
...
function deleteUsers( vanityUrls ) {
  ...
  .then( () => {
    return Results.create( scopeBreakingTrash )
  })
  .then( fuckingwhat => {
    return fuckingwhat;
  })
