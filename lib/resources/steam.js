...
function getMostPopularGameFromVanityURLs( communityURLs, criteria ) {
  ...
  .then( usersPlayTimes => {
    ...
    if( criteria === 'ownership' ) {
      return util.findMostPopularGameByOwnership( merged );
    } else {
      return util.findMostPopularGameByPlaytime( merged );
    }
...
module.exports = {
  ...
  getMostPopularGameFromVanityURLs
}
