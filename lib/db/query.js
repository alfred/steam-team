...
module.exports = {
  ...
  select( table, where ) {
    return new Promise( ( resolve, reject ) => {
      return this.db().then( db => {
        ...
        let wurr = clauses.length > 0 ? 'WHERE' : ''

        let sql = `SELECT * FROM ${ table } ${ wurr } ${ clauses.join(' AND ') }`;
   ...
   * @param  {String} table - name of the table to insert into
   * @param  {Object} where - object to match in the select
   * @return {Object}       - I think the deleted obj who knows
   */
  delete( table, where ) {
    return new Promise( ( resolve, reject ) => {
      return this.db().then( db => {
  ...
  upsert( table, data ) {
    return new Promise( ( resolve, reject ) => {
      return this.db().then( db => {
        const upsertPrefix = 'ON DUPLICATE KEY UPDATE';

        let updateKeys = Object.keys( Object.assign( {}, data ) );
        let newValues = [];
        let updateClause = updateKeys.map( key => {
          newValues.push( data[ key ] );
          return `${ key } = ?`;
        });

        let sql = `INSERT INTO ${ table } SET ${ updateClause }` +
          ` ${ upsertPrefix } ${ updateClause }`;

        db.query( sql, newValues.concat( newValues ), ( err, res ) => {
          if ( err ) {
            return reject( err );
          }
